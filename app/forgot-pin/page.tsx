...
export default function ForgotPinPage() {
  ...
  const submit = useCallback(async () => {
    ...
    if (!email.trim()) { setMsg("이메일을 입력해 주세요."); setBusy(false); return; }
    try {
      ...
      if (!res.ok || !json.ok) { setMsg(json.error ?? "요청에 실패했습니다."); return; }
      setStep("done");
  ...
  return (
    <main style={mainStyle}>
      <div style={cardStyle}>
        <h1 style={{ margin: "0 0 0.25rem", fontSize: "1.5rem", color: "var(--text-primary)" }}>PIN 찾기</h1>
        {step === "done" ? (
          <>
          ...
          <>
            ...
            <label style={lab}>
              전화번호
              <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="01012345678" style={inp} />
            </label>
            <label style={lab}>
              이메일
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="example@email.com" autoComplete="email" style={inp} />
            </label>
            <button type="button" onClick={submit} disabled={busy} style={btnStyle(busy)}>
